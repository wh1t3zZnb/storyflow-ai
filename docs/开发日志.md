...
- 阶段13：加入登录模块(待完善)
  - 引入 Supabase 客户端配置 (`src/utils/supabase.js`)。
  - 新增 `AuthContext` (`src/context/AuthContext.jsx`) 用于管理用户登录状态。
  - 新增 `LoginModal` (`src/components/Auth/LoginModal.jsx`) 用于用户登录/注册。
  - 新增 `UserMenu` (`src/components/Auth/UserMenu.jsx`) 用于显示用户信息和登出。
  - 在 `main.jsx` 中包裹 `AuthProvider`，并在 `App.jsx` 中集成登录 UI。
  - 状态：功能待完善，初步集成代码结构。
